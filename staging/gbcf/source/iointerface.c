/*
  io_mmcf.c based on
	...
	Universal CF DLDI driver for all known CF based slot-2 carts.
	The known carts that are compatible with this driver:
	Max Media Dock (MMC)
	Movie Player CF (MPCF)
	M3CF
	SuperCard CF (SCCF)
	
	All these cards use same code base but with different registers with a couple using additional mode switch registers.
	This driver rebuilt by Apache Thunder with help from the following:
		- Support for more then 256 sectors in a read added courtasy of Robz8.
		- setFastCNT optimizations courtay of Ausar.
*/
